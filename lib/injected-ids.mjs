...
export function readInjectedMarker(file, { sessionId, maxAgeMs } = {}) {
  const empty = { ids: [], count: 0, upsCount: 0, fresh: false };
  try {
    const { ids, ts, count, upsCount, session } = JSON.parse(readFileSync(file, 'utf8'));
    ...
    return { ids, count: count || 0, upsCount: upsCount || 0, fresh: true };
  } catch {
    return empty;
  }
}

...
// mid-task. Nothing read them. Measured 2026-09-21: 18 such files across 9 projects on this
 ...
 *
 * Under vitest the default resolves to the maintainer's real repo, which carries five
 ...
 *          or the note has nothing left to do.
 */
export function readPausedNote({ projectPath = defaultProjectPath(), maxItems = 5 } = {}) {
  ...
  try {
    for (const name of readdirSync(tasksDir)) {
      ...
      try {
        const mtime = statSync(join(tasksDir, name)).mtimeMs;
  ...
  for (const heading of REMAINING_HEADINGS) {
    ...
    const flush = () => {
      ...
      if (item) items.push(item.length > MAX_ITEM_CHARS ? item.slice(0, MAX_ITEM_CHARS) : item);
      current = '';
    };

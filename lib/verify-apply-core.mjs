...
/**
 * A short digest of what an approval covers: the proposed changes AND the state of every row
 * they were checked against. The dry run prints it; --apply refuses without it. A proposals
 * file edited after the user approved, or a row a hook rewrote in between, changes the digest.
 * @returns {string} 16 hex chars
 */
export function planDigest(plan, project) {
  const payload = {
    project,
    entries: plan.map(({ before, files, ...entry }) => ({
      entry,
      before: pick(before, TRACKED_COLUMNS),
      files,
    })),
  };
...
export function applyVerifyPlan(db, plan, { now = new Date(), expectRows = null } = {}) {
  ...
  const tx = db.transaction(() => {
    ...
    for (const p of plan) {
      ...
      // and only ADD fields, so they still reach the row.
...
export function runVerifyApply(db, plan, { backupDir, now = new Date() }) {
  ...
  const backup = {
    ...
    project,
    digest: planDigest(plan, project),
    rows,
    applied: null,
    undone_at: null,
  };
  ...
  backup.applied = plan.map((p) => {
    ...
    return {
      ...
      evidence: p.evidence,
      newId,
      after: pick(getRow(db, p.id), TRACKED_COLUMNS),
      replacementAfter: newId ? pick(getRow(db, newId), TRACKED_COLUMNS) : null,
    };
  });
  try {
    atomicWriteFileSync(backupPath, JSON.stringify(backup, null, 1));

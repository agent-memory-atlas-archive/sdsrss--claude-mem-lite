...
/**
 * True when `installDir` holds ANY managed entry point — i.e. code was deployed here at
 * some point, even if it is now incomplete.
 *
 * The complement of `hasManagedCodeInstall` above is not one population but two, and they
 * take OPPOSITE remedies: an install that is DAMAGED (some entry points survive) is what
 * `repair` exists for, while one that was never deployed here needs `install`. `repair`
 * runs from `<installDir>/cli.mjs`, so prescribing it where nothing was deployed names a
 * binary that cannot start. Same list as above, deliberately — `every` vs `some` is the
 * whole difference, and deriving both from one array keeps them from drifting apart.
 *
 * @param {string} installDir
 * @returns {boolean}
 */
export function hasAnyManagedCode(installDir) {
  if (!installDir || !existsSync(installDir)) return false;
  return MANAGED_ENTRY_POINTS.some((f) => existsSync(join(installDir, f)));
}

...
export function nativeBindingRepairHint(root) {
  // shellWord, not "…": INSTALL_DIR / a plugin-cache root can contain spaces (and, under
  // double quotes, a `$` or backtick still expands), and a `cd` that splits hands the user a
  // command that fails on exactly the machines least able to debug it.
  const npmPair = `cd ${shellWord(root)} && ${NATIVE_BINDING_REBUILD_CMD} && ${NATIVE_BINDING_SOURCE_BUILD_CMD}`;
  ...
  return `node ${shellWord(cli)} rebuild-binding   (or, without the CLI: ${npmPair})`;
}

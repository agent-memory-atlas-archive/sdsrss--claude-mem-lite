...
export function dbUnusableRemedy(dbPath) {
  const clear = `rm -f ${shellWord(`${dbPath}-wal`)} ${shellWord(`${dbPath}-shm`)}`;
  ...
  if (snap.snapshots.length === 0) {
    return {
      kind: 'set-aside',
      command: `${clear} && mv ${shellWord(dbPath)} ${shellWord(`${dbPath}.corrupt`)}`,
      snapshotCount: 0,
      note:
  ...
  return {
    kind: 'restore',
    command: `${clear} && cp ${shellWord(newest.path)} ${shellWord(dbPath)}`,

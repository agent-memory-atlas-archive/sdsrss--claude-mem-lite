...
export function supersededNotice(row) {
  ...
  if (by)
    return `⚠ RETRACTED — superseded by ${by}. Read ${by} instead; the fields below are the withdrawn version.`;
  // String markers written by /verify (lib/verify-apply-core.mjs). Named, because "auto-dedup
  // or merge" would send a reader looking for a duplicate that does not exist.
  if (row.superseded_by === 'verify-retired') {
    return '⚠ RETRACTED — retired by /verify as out of date, with no replacement. The fields below are the withdrawn version.';
  }
  if (row.superseded_by === 'verify-undone') {
    return '⚠ RETRACTED — a /verify correction withdrawn by its undo; the original it replaced is live again. The fields below are the withdrawn version.';
  }
  return '⚠ RETRACTED — superseded (auto-dedup or merge). The fields below are the withdrawn version.';
}

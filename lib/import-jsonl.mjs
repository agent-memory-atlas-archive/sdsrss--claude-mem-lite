 ...
 *
 * One scrub per side, and it happens HERE, so both sites get the same string by
 * construction and neither may wrap this call in another scrub. An earlier cut scrubbed
 * here AND left the storage side's `scrubRecord` scrubbing `title` again — equal only while
 * `scrubSecrets` is idempotent, which it was not until D#52 (`Bash: deploy --token ghp_…
 * secret: v` imported three rows for three runs). The fix then returned the RAW title for
 * both sites to scrub (D#46).
 *
 * Scrub FIRST, cut second (D#47 P3-8). Cutting first handed the scrubber a token the cut
 * had split, too short for its own pattern (`ghp_` needs 30+ characters), and the prefix
 * was stored in plaintext — up to 29 of a GitHub token's 36 secret characters, at 29 of 41
 * cut positions. The whole title goes through the scrubber; the cap counts from the end of
 * the `<tool>: ` prefix.
 */
...
function importedObsTitle(toolUse) {
  ...
  const prefix = `${toolUse?.name || 'unknown'}: `;
  const scrubbed = scrubRecord('observations', { title: `${prefix}${detail}` }).title;
  return scrubbed.slice(0, prefix.length + IMPORTED_TITLE_DETAIL_MAX);
}
...
function importPrompt(db, ev, project, seenPrompts) {
  ...
  // Scrub, THEN cap: cutting first handed the scrubber a token the cap had split, too short
  // for its own pattern, and stored the prefix (same order bug as importedObsTitle).
...
function importToolPair(db, toolUse, toolResult, project) {
  ...
  // Each part is scrubbed BEFORE its 4000-character cap, for the reason importedObsTitle
  // gives: a cap that splits a token leaves a prefix too short for the scrubber's pattern.
  // The body is therefore kept out of the scrubRecord call below, so it is scrubbed once.
  const inputJson = scrubSecrets(
    typeof toolUse.input === 'object' ? JSON.stringify(toolUse.input) : String(toolUse.input ?? ''),
  ).slice(0, 4000);
  const resultText =
    typeof toolResult?.content === 'string'
      ? scrubSecrets(toolResult.content)
      : scrubSecrets(JSON.stringify(toolResult?.content ?? '')).slice(0, 4000);
  ...
  const safe = {
    ...
    // Outside scrubRecord on purpose: body and title were each already scrubbed, once.
    text: body,
    narrative: body,
    // The title: this string IS the cross-run dedup key — `importedObsTitle` is the single home for
...
export async function importJsonl(db, path, { project }) {
  ...
  const tryImportToolPair = (useEv, resultEv) => {
    ...
    // the seenObs set seeded from the DB. The same function as the storage site, scrub and
    // cut included, so the scrubber cannot rewrite one and not the other.
